interface ClinicalAuditRecord {
  ...
  const COLUMN_HEADERS: Record<keyof ClinicalAuditRecord, string> = {
  ...
  export function exportToCSV(data: ClinicalAuditRecord[], filename: string) {
    ...
    const fields = Object.keys(COLUMN_HEADERS) as (keyof ClinicalAuditRecord)[];
  ...
  export const socioeconomicOptions = [
    { value: "low", label: "Low" },
...
// Extract facility values for validation
export const facilityValues = facilityOptions.map(option => option.value);

// Create a tuple type for zod enum
export const facilityValuesTuple = facilityValues as [string, ...string[]];
